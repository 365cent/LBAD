...
class SwAVPrototypes(nn.Module):
    ...
    def forward(self, features):
        ...
        scores = torch.matmul(features, prototypes.T) / self.temperature
        return scores
...
class DynamicAugmentation(nn.Module):
    ...
    def __init__(
        self, input_dim: int, max_noise: float = 0.1, max_dropout: float = 0.3
    ):
        super().__init__()
...
def multilabel_consistency_loss(
    predictions: torch.Tensor, temperature: float = 2.0
) -> torch.Tensor:
    """
    ...
    with torch.no_grad():
        high_sim_mask = (similarity_matrix > 0.7) & (
            ~torch.eye(batch_size, device=predictions.device, dtype=bool)
        )
...
def class_balance_regularization(
    predictions: torch.Tensor, target_distribution: torch.Tensor = None
) -> torch.Tensor:
    """
    ...
    kl_loss = F.kl_div(
        torch.log(current_dist), target_distribution, reduction="batchmean"
    )

    return kl_loss


def multilabel_contrastive_loss(
    ...
    for i in range(batch_size):
        ...
        positive_mask = (
            (torch.matmul(anchor_labels, pred_binary.T) > 0)
            & (torch.arange(batch_size, device=embeddings.device) != i)
        ).float()
        ...
        if positive_mask.sum() > 0 and negative_mask.sum() > 0:
            ...
            hardest_neg_sim = (
                neg_sim[neg_sim > 0].max() if neg_sim.sum() > 0 else -1.0
            )  # Use -1.0 if no negatives
            ...
            loss += F.mse_loss(
                pos_sim[pos_sim > 0], torch.ones_like(pos_sim[pos_sim > 0])
            )
...
def generate_reconstruction_anomaly_scores(
    model: nn.Module,
    embeddings: torch.Tensor,
    batch_size: int = 256,
        ...
        for i in range(0, len(embeddings), batch_size):
            try:
                ...
                if "reconstructed" in outputs:
                    ...
                    print(
                        "⚠️  No reconstruction output, using label predictions as proxy..."
                    )
                    label_output = torch.sigmoid(outputs["labels"])
                    anomaly_proxy = (
                        1.0 - label_output.max(dim=1)[0]
                ...
                if "out of memory" in str(e) or "misaligned address" in str(e):
                    print(
                        f"⚠️  Memory/alignment error at batch {i // batch_size + 1}, trying smaller batch..."
                    )

                    # Try with smaller sub-batches
                    sub_batch_size = batch_size // 4
                    for j in range(
                        i, min(i + batch_size, len(embeddings)), sub_batch_size
                    ):
                        try:
                            sub_batch = (
                                embeddings[j : j + sub_batch_size]
                                .contiguous()
                                .to(device)
                            )
                            ...
                            if "reconstructed" in sub_outputs:
                                ...
                                sub_error = F.mse_loss(
                                    sub_recon, sub_batch, reduction="none"
                                ).mean(dim=1)
                            else:
                                sub_error = (
                                    1.0
                                    - torch.sigmoid(sub_outputs["labels"]).max(dim=1)[0]
                                )
                            ...
                            recon_errors.append(
                                sub_error.detach().clone().cpu().numpy()
                            )
...
def optimize_advanced_thresholds(
    ...
    for i, cls in enumerate(classes):
        ...
        if pos_samples == 0:
            ...
            for threshold in np.linspace(
                0.05, 0.95, 50
            ):  # More granular for rare classes
...
class UnsupervisedMultiLabelTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.label_attention = nn.MultiheadAttention(
            latent_dim, 8, dropout=dropout, batch_first=True
        )
    ...
    def forward(
        self, x, return_vae_params: bool = False, apply_augmentation: bool = True
    ):
        batch_size = x.shape[0]
        ...
        if return_vae_params and self.use_vae:
            outputs.update({"vae_mu": vae_mu, "vae_logvar": vae_logvar})
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path, log_type: str, config: SystemConfig):
        ...
        log_file = (
            output_dir / f"training_{log_type}_{config.node_name}_{config.job_id}.log"
        )
        logging.basicConfig(
            level=logging.INFO,
    ...
    def start_training(self, total_epochs: int, total_batches_per_epoch: int = 0):
        ...
        print(
            f"🚀 Training started for {self.log_type}: {total_epochs} epochs, ~{total_batches_per_epoch} batches/epoch"
        )
    ...
    def update_batch_progress(
        ...
        should_update = (
            (batch_idx % self.progress_update_interval == 0)
            or is_epoch_end
            or (current_time - self.last_progress_update)
            > 30  # At least every 30 seconds
        )
        ...
        if should_update and self.total_batches_per_epoch > 0:
            ...
            total_completed_batches = (
                completed_epochs * self.total_batches_per_epoch
                + completed_batches_this_epoch
            )
            total_batches = self.total_epochs * self.total_batches_per_epoch

            overall_progress_pct = (
                (total_completed_batches / total_batches) * 100
                if total_batches > 0
                else 0
            )
            epoch_progress_pct = (
                completed_batches_this_epoch / self.total_batches_per_epoch
            ) * 100
            ...
            if loss_info:
                ...
                meaningful_losses = {}
                if "total" in loss_info and not (
                    loss_info.get("skipped", False)
                    or loss_info.get("oom", False)
                    or loss_info.get("nan_skip", False)
                ):
                ...
                if meaningful_losses:
                    loss_str = " | ".join(
                        [
                            ...
                            for k, v in meaningful_losses.items()
                        ]
                    )
                    loss_str = f" | {loss_str}"
            ...
            print(
                f"📊 Progress ({self.log_type}): {overall_progress_pct:.1f}% | "
    ...
    def update_epoch_progress(self, epoch: int, epoch_time: float):
        ...
        if len(self.epoch_times) >= 2:
            avg_epoch_time = np.mean(
                self.epoch_times[-5:]
            )  # Use last 5 epochs for better estimate
            ...
            print(
                f"✅ Epoch {epoch + 1}/{self.total_epochs} completed ({overall_progress_pct:.1f}%) | "
    ...
    def log_step(self, step: str, data: Dict[str, Any]):
        ...
        step_file = (
            self.output_dir
            / f"step_{step.lower().replace(' ', '_')}_{self.log_type}_{self.config.node_name}_{self.config.job_id}.json"
        )
    ...
    def log_metrics(self, epoch: int, metrics: Dict[str, float]):
        ...
        metrics_file = (
            self.output_dir
            / f"metrics_{self.log_type}_{self.config.node_name}_{self.config.job_id}.json"
        )
        with open(metrics_file, "w") as f:
            json.dump(self.metrics, f, indent=2)

    def save_checkpoint_progress(
        self, epoch: int, total_loss: float, additional_info: Dict[str, Any] = None
    ):
        ...
        progress_file = (
            self.output_dir
            / f"progress_{self.log_type}_{self.config.node_name}_{self.config.job_id}.json"
        )
...
def load_and_preprocess_data(
    ...
    if not classes:
        ...
        true_labels = np.ones(
            (len(embeddings), 1), dtype=np.float32
        )  # All samples are normal
        tracker.log_step(
            "No Labels Found - Using Normal Class",
            {
                "n_samples": len(embeddings),
    ...
    if sample_size is not None and sample_size < len(embeddings):
        ...
        # Using full dataset
        print(
            f"📊 Using full dataset: {len(embeddings):,} samples ({embeddings.nbytes / (1024**3):.1f} GB)"
        )
        tracker.log_step(
...
def focal_loss(
    logits: torch.Tensor, targets: torch.Tensor, alpha: float = None, gamma: float = 2.0
) -> torch.Tensor:
    """
    ...
    if alpha is None:
        ...
        alpha_t = targets * pos_weights.unsqueeze(0) + (
            1 - targets
        ) * neg_weights.unsqueeze(0)
    else:
        alpha_t = alpha
...
def prototype_margin_loss(
    ...
    distances = torch.cdist(
        z_prototype.unsqueeze(1), class_prototypes.unsqueeze(0)
    ).squeeze(1)
...
def confidence_regularization_loss(
    predictions: torch.Tensor, confidence_target: float = 0.8
) -> torch.Tensor:
    """
    ...
    confidence_loss = F.mse_loss(
        confidence, torch.full_like(confidence, confidence_target)
    )
...
def generate_pseudo_labels(
    ...
    for i in range(n_samples):
        ...
        pseudo_labels[i][top_indices] = np.minimum(
            pseudo_labels[i][top_indices] + confidence_boost, 1.0
        )
...
def negative_class_suppression_loss(
    logits: torch.Tensor,
...
def contrastive_loss(
    ...
    all_similarities = torch.clamp(
        all_similarities, min=-10, max=10
    )  # Prevent overflow
...
def generate_input_embeddings_for_logs(
    logs: List[str], target_dim: int, device: str
) -> Optional[np.ndarray]:
    """
...
def generate_enhanced_logbert_embeddings_for_logs(
    logs: List[str], device: str
) -> np.ndarray:
    ...
    with torch.no_grad():
        for i in range(0, len(logs), batch_size):
            ...
            for j in range(cls_attention.shape[0]):
                ...
                if len(valid_attention) > 1:
                    ...
                    if len(top_values) < 10:
                        top_values = np.pad(
                            top_values, (0, 10 - len(top_values)), "constant"
                        )
                else:
            ...
            combined = np.hstack(
                [cls_embeddings, mean_embeddings, max_embeddings, attention_features]
            )
...
def save_trained_model(
    ...
    model_path = (
        models_dir / f"transformer_{log_type}_{config.node_name}_{config.job_id}.pth"
    )
    ...
    print(
        f"🏗️  Architecture: {model_to_save.transformer_layers} layers, {model_to_save.attention_heads} heads, {model_to_save.latent_dim}D latent"
    )
    print(
        f"🎯 Evaluation: Use 'python src/evaluate_transformer.py --log-type {log_type}' for full evaluation"
    )
...
def calculate_comprehensive_metrics(
        ...
        metrics["most_frequent_classes"] = [
            {
                "class": cls,
                "count": int(count),
                "percentage": float(count / len(binary_predictions) * 100),
            }
    ...
    if y_true is not None:
        ...
        metrics["macro_f1"] = float(
            f1_score(y_true, binary_predictions, average="macro", zero_division=0)
        )
        metrics["micro_f1"] = float(
            f1_score(y_true, binary_predictions, average="micro", zero_division=0)
        )
        metrics["weighted_f1"] = float(
            f1_score(y_true, binary_predictions, average="weighted", zero_division=0)
        )
        metrics["samples_f1"] = float(
            f1_score(y_true, binary_predictions, average="samples", zero_division=0)
        )
        ...
        metrics["jaccard_score_macro"] = float(
            jaccard_score(y_true, binary_predictions, average="macro", zero_division=0)
        )
        metrics["jaccard_score_micro"] = float(
            jaccard_score(y_true, binary_predictions, average="micro", zero_division=0)
        )
        ...
        if probs is not None:
            ...
            metrics["correct_prediction_confidence_mean"] = (
                float(correct_confidences.mean())
                if len(correct_confidences) > 0
                else 0.0
            )
            metrics["incorrect_prediction_confidence_mean"] = (
        ...
        if probs is not None:
            ...
            metrics["high_confidence_percentage"] = float(
                high_conf_mask.sum() / probs.size * 100
            )
...
def create_comprehensive_visualizations(
    ...
    try:
        ...
        fig.suptitle(
            f"Transformer Classification Analysis - {log_type.upper()}", fontsize=16
        )
        ...
        axes[0, 2].hist(
            predictions_viz.flatten(), bins=50, alpha=0.7, edgecolor="black"
        )
        ...
        plt.savefig(
            output_dir
            / f"classification_analysis_{log_type}_{config.node_name}_{config.job_id}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
...
def create_visualization(
    ...
    try:
        ...
        plt.savefig(
            output_dir
            / f"visualization_{log_type}_{config.node_name}_{config.job_id}.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def advanced_pseudo_label_generation(
    embeddings: np.ndarray,
    classes: List[str],
    true_labels: np.ndarray = None,
    epoch: int = 0,
    ...
    if true_labels is not None:
        ...
        pseudo_labels = pseudo_labels * (0.95 + 0.05 * progress) + noise * (
            1 - progress
        )
        ...
        if np.any(unlabeled_mask):
            ...
            confident_labels = (confident_labels > (1 - base_confidence)).astype(
                float
            ) * 0.8 + 0.1
            pseudo_labels[unlabeled_mask] = confident_labels
    else:
        ...
        pseudo_labels = np.random.rand(n_samples, n_classes).astype(np.float32)
        pseudo_labels = (pseudo_labels > (1 - base_confidence)).astype(
            float
        ) * 0.7 + 0.15
    ...
    for i in range(0, n_samples, chunk_size):
        ...
        for j in range(len(chunk_i)):
            ...
            if (
            ...
            ):  # Ensure there's more than just the sample itself
                ...
                if len(similar_indices) > 0:  # Check again after removing self
                    ...
                    sim_weights = sim_weights / (
                        sim_weights.sum() + 1e-8
                    )  # Add epsilon for stability
                    propagated_labels = np.average(
                        pseudo_labels[similar_indices], axis=0, weights=sim_weights
                    )
    ...
    for i in range(n_samples):
        ...
        pseudo_labels[i][top_indices] = np.minimum(
            pseudo_labels[i][top_indices] + boost_factor, 0.95
        )
    ...
    pseudo_labels = np.nan_to_num(
        pseudo_labels, nan=min_conf, posinf=max_conf, neginf=min_conf
    )

    return pseudo_labels


def enhanced_adaptive_thresholding(
    predictions: np.ndarray,
    classes: List[str],
    true_labels: np.ndarray = None,
    ...
    for class_idx in range(n_classes):
        ...
        if true_labels is not None and class_idx < true_labels.shape[1]:
            ...
            rarity_factor = 1.0 - (
                class_rarity[class_idx] / (class_rarity.max() + 1e-8)
            )
            ...
            adaptive_thresholds[class_idx] = np.clip(
                adaptive_thresholds[class_idx], 0.1, 0.9
            )

    # Step 4: Apply thresholds and assign binary predictions
    binary_predictions = np.zeros_like(predictions, dtype=int)
    for class_idx in range(n_classes):
        binary_predictions[:, class_idx] = (
            predictions[:, class_idx] >= adaptive_thresholds[class_idx]
        ).astype(int)
...
def enhanced_adaptive_thresholding(
    predictions: np.ndarray, classes: List[str], true_labels: np.ndarray = None
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Enhanced adaptive thresholding combining global information for better F1.
    Simplified for better speed and stability.
...
def split_for_unsupervised_eval(
    embeddings: np.ndarray,
...
def evaluate_transformer_model(
    ...
    if true_labels is not None and n_val > 0:
        ...
        optimal_thresholds = np.clip(
            class_frequencies, 0.2, 0.8
        )  # Balance based on class frequency
        ...
        adaptive_thresholds = np.mean(predictions, axis=0) + 0.5 * np.std(
            predictions, axis=0
        )
...
def calculate_unsupervised_metrics(
    y_pred: np.ndarray, y_prob: np.ndarray, classes: List[str], thresholds: np.ndarray
) -> Dict[str, Any]:
        ...
        results["most_frequent_classes"] = [
            {
                "class": cls,
                "count": int(count),
                "percentage": float(count / len(y_pred) * 100),
            }
            for cls, count in class_freq_pairs[:10]
        ]
    ...
    results["high_confidence_percentage"] = float(
        high_conf_mask.sum() / y_prob.size * 100
    )
...
def generate_classification_report(
    ...
    report_lines = []
    report_lines.append(
        f"TRANSFORMER Multi-Label Classification Report - {log_type.upper()}"
    )
    ...
    if (
        ...
        report_lines.append("-" * 40)
        report_lines.append(
            f"{'Class':<25} {'F1':<8} {'Precision':<10} {'Recall':<8} {'Support':<8} {'Threshold':<10}"
        )
        report_lines.append("-" * 80)
        ...
        for i, cls_name in enumerate(classes):
            ...
            recall = (
                ...
                if i < len(results["per_class_recall"])
                else 0.0
            )
            support = (
                int(results["per_class_support"][i])
                if i < len(results["per_class_support"])
                else 0
            )
            threshold = (
                results["optimal_thresholds"][i]
                if i < len(results["optimal_thresholds"])
                else 0.5
            )

            report_lines.append(
                f"{cls_name:<25} {f1:<8.3f} {precision:<10.3f} {recall:<8.3f} {support:<8} {threshold:<10.3f}"
            )
        ...
        report_lines.append("-" * 40)
        report_lines.append(
            f"Prediction Confidence: {results['prediction_confidence_mean']:.4f}"
        )
        report_lines.append(
            f"Confidence Std: {results['prediction_confidence_std']:.4f}"
        )
        report_lines.append(
            f"Avg Labels/Sample: {results['avg_labels_per_sample']:.2f}"
        )
        report_lines.append(
            f"High Conf Predictions: {results['high_confidence_percentage']:.1f}%"
        )
        report_lines.append("")
        ...
        if "most_frequent_classes" in results:
            ...
            for i, cls_info in enumerate(results["most_frequent_classes"][:10]):
                report_lines.append(
                    f"  {i + 1:2d}. {cls_info['class']:<25} {cls_info['count']:>6} ({cls_info['percentage']:>5.1f}%)"
                )
    ...
    report_lines.append(
        f"Samples with multiple labels: {results['samples_with_multiple_labels']}"
    )
    report_lines.append(
        f"Average labels per sample: {results['avg_labels_per_sample']:.3f}"
    )
    report_lines.append(
        f"Std labels per sample: {results['std_labels_per_sample']:.3f}"
    )
    report_lines.append(
        f"Labels per sample range: {results['min_labels_per_sample']} - {results['max_labels_per_sample']}"
    )
    ...
    report_lines.append(
        "Features: Multi-label consistency, class balance regularization,"
    )
    ...
    report_path = (
        output_dir
        / f"transformer_classification_report_{log_type}_{config.node_name}_{config.job_id}.txt"
    )
...
def save_evaluation_results(
    ...
    results_path = (
        output_dir
        / f"transformer_evaluation_{log_type}_{config.node_name}_{config.job_id}.pkl"
    )
    ...
    if (
        results["evaluation_type"] == "supervised"
        and results.get("per_class_f1") is not None
    ):
        sklearn_report_path = (
            output_dir
            / f"sklearn_report_{log_type}_{config.node_name}_{config.job_id}.txt"
        )
        ...
        try:
            ...
            sklearn_report += (
                "Use the transformer_classification_report for detailed metrics.\n"
            )
    ...
    if (
        ...
        if "most_frequent_classes" in results:
            ...
            for i, cls_info in enumerate(results["most_frequent_classes"][:5]):
                print(
                    f"  {i + 1}. {cls_info['class']:<25} {cls_info['percentage']:5.1f}%"
                )

    print(f"{'=' * 80}")


def generate_per_class_accuracy_report(
    results_dir: Path,
    log_type: str,
    config: SystemConfig,
    ...
    if predictions is None or binary_predictions is None or classes is None:
        ...
        results_file = (
            results_dir / f"results_{log_type}_{config.node_name}_{config.job_id}.pkl"
        )
            ...
            try:
                with open(label_file, "rb") as f:
                    ...
                    if isinstance(label_data, dict) and "vectors" in label_data:
                        ...
                        true_labels = (
                            label_data if isinstance(label_data, np.ndarray) else None
                        )
            except Exception as e:
    ...
    if true_labels is not None and len(true_labels) > 0:
        ...
        try:
            ...
            micro_f1 = f1_score(
                test_labels, test_predictions, average="micro", zero_division=0
            )
            ...
            report_lines.append("-" * 80)
            report_lines.append(
                f"{'Class':<20} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1':<10} {'Support':<10}"
            )
            report_lines.append("-" * 80)
            ...
            for i, cls_name in enumerate(classes):
                ...
                report_lines.append(
                    f"{cls_name:<20} {class_accuracy:<10.4f} {precision:<10.4f} {recall:<10.4f} {f1:<10.4f} {support:<10}"
                )
    ...
    if true_labels is None:
        ...
        report_lines.append(
            f"{'Class':<20} {'Predictions':<12} {'Percentage':<12} {'Avg Conf':<12} {'Std Conf':<10}"
        )
        report_lines.append("-" * 70)
        ...
        for i, cls_name in enumerate(classes):
            ...
            report_lines.append(
                f"{cls_name:<20} {count:<12} {percentage:<12.2f} {avg_conf:<12.4f} {std_conf:<10.4f}"
            )
    ...
    report_lines.append("-" * 40)
    report_lines.append(
        f"Samples with 0 labels: {(labels_per_sample == 0).sum()} ({(labels_per_sample == 0).mean() * 100:.1f}%)"
    )
    report_lines.append(
        f"Samples with 1 label:  {(labels_per_sample == 1).sum()} ({(labels_per_sample == 1).mean() * 100:.1f}%)"
    )
    report_lines.append(
        f"Samples with 2 labels: {(labels_per_sample == 2).sum()} ({(labels_per_sample == 2).mean() * 100:.1f}%)"
    )
    report_lines.append(
        f"Samples with 3+ labels: {(labels_per_sample >= 3).sum()} ({(labels_per_sample >= 3).mean() * 100:.1f}%)"
    )
    ...
    if len(classes) <= 10:  # Only for manageable number of classes
        ...
        for i, (combo, count) in enumerate(combo_counts.most_common(10)):
            ...
            report_lines.append(
                f"{i + 1:2d}. {', '.join(active_classes):<35} {count:>6} ({percentage:>5.1f}%)"
            )
    ...
    report_path = (
        results_dir
        / f"per_class_accuracy_report_{log_type}_{config.node_name}_{config.job_id}.txt"
    )
...
def generate_per_class_report_standalone(
    log_type: str = "wp-error", node_name: str = "gra6", job_id: str = "29386936"
):
    """
...
def load_training_checkpoint(
    log_type: str, training_hash: str
    ...
    try:
        ...
        try:
            ...
            with torch.serialization.safe_globals(
                ["numpy.core.multiarray.scalar", "numpy.ndarray", "numpy.dtype"]
            ):
        ...
        # Validate checkpoint
        if (
            checkpoint_data["log_type"] == log_type
            and checkpoint_data["training_hash"] == training_hash
        ):
            age_hours = (time.time() - checkpoint_data["timestamp"]) / 3600
            epoch = checkpoint_data["epoch"]
            print(
                f"📂 Found training checkpoint: epoch {epoch} (age: {age_hours:.1f}h)"
            )
...
def process_log_type_with_args(
    ...
    if result_status["complete"] and not force_restart:
        ...
        try:
            ...
            results_path = (
                output_dir
                / f"results_{log_type}_{config.node_name}_{config.job_id}.pkl"
            )
            ...
            if not result_status["visualizations"]:
                ...
                embeddings, classes, _, _ = load_and_preprocess_data(
    ...
    try:
        ...
        with Halo(text=f"Loading data for {log_type}...", spinner="dots") as spinner:
            embeddings, classes, C, scaler = load_and_preprocess_data(
                log_type, config, tracker, sample_size
            )
            spinner.succeed(
                f"Data loaded: {embeddings.shape[0]} samples, {embeddings.shape[1]} features"
            )
        ...
        print(f"📂 Outputs:")
        print(
            f"   Model: models/transformer_{log_type}_{config.node_name}_{config.job_id}.pth"
        )
        print(f"   Predictions: results/{log_type}/predictions.pkl")
        print(f"")
        print(f"🎯 Next steps:")
        print(
            f"   Evaluation: python src/evaluate_transformer.py --log-type {log_type}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Transformer-based Unsupervised Multi-Label Learning - Resumeable"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force-restart",
        action="store_true",
        help="Force restart processing (ignore existing results)",
    )
    parser.add_argument(
        "--clean-checkpoints",
        action="store_true",
        help="Clean up all training checkpoints before starting",
    )
    parser.add_argument(
        "--sample-size",
        type=int,
    ...
    try:
        ...
        if config.rank == 0:
            ...
            print(
                f"  Memory optimizations: {'CUDA conservative' if config.device == 'cuda' else 'Standard'}"
            )
            print(
                f"  Supports: FastText (300D), BERT CLS (768D), Enhanced LogBERT (2314D)"
            )
        ...
        if args.log_type:
            if args.log_type in available_types:
                ...
                print(
                    f"❌ Log type '{args.log_type}' not found in available types: {available_types}"
                )
                return
        ...
        for idx, log_type in enumerate(available_types, 1):
            if config.rank == 0:
                ...
                if log_type in embedding_analysis:
                    ...
                    print(
                        f"Samples: {info['n_samples']:,} | Memory: {info['memory_mb']:.1f} MB"
                    )
                    print(f"{'=' * 60}")
                else:
                    print(f"\n{'=' * 60}")
            ...
            process_log_type_with_args(
                log_type, config, args.force_restart, args.sample_size
            )
            ...
            if config.rank == 0:
                ...
                if remaining_types > 0:
                    ...
                    print(
                        f"Progress: {idx}/{total_types} ({idx / total_types * 100:.1f}%)"
                    )
                    print(
        ...
        if config.rank == 0:
            ...
            print(
                f"✅ Resumeable: Training checkpoints saved for interrupted processing"
            )
            print(f"📁 Results saved to: {RESULTS_DIR}/")
            print(f"🤖 Models saved to: {MODELS_DIR}/")
            print(
                f"🏷️  Labels saved in evaluation format to: {RESULTS_DIR}/*/label_*.pkl"
            )
            print(
                f"📊 Classification reports saved to: {RESULTS_DIR}/*/transformer_classification_report_*.txt"
            )
            print(
                f"📈 Per-class accuracy reports saved to: {RESULTS_DIR}/*/per_class_accuracy_report_*.txt"
            )
            ...
            print(
                f"  - Supervised metrics (when true labels available): F1, Precision, Recall, Hamming Loss, Jaccard"
            )
            print(f"  - Per-class performance with optimized thresholds")
            print(
                f"  - Per-class accuracy, confusion matrices, sensitivity/specificity"
            )
