...
Key Improvements:
- Uses the pre-trained GoogleNews Word2Vec model for consistent embeddings
- Embeds logs as Word2Vec vectors for better semantic representation
...
def load_pretrained_word2vec() -> Optional[KeyedVectors]:
    ...
    try:
        ...
        spinner.text = "Trying alternative GloVe model (glove-wiki-gigaword-300)"
        try:
            model = api.load("glove-wiki-gigaword-300")
            spinner.succeed("Loaded alternative GloVe model (glove-wiki-gigaword-300)")
            return model
        except Exception as secondary_exc:
            spinner.fail(f"Failed to load pre-trained embeddings: {secondary_exc}")
            print("Consider installing gensim-data resources manually or check internet connection")
            return None
...
def embed_text(
    ...
    """Generate embedding for tokenized text using Word2Vec vectors."""
    if not tokens:
    ...
    if 'binary_labels' in df.columns and len(df['binary_labels']) > 0:
        ...
        if valid_vectors:
            ...
            with open(output_dir / attack_info_filename, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Embedding dimension: 300D (Word2Vec vectors)\n")
            ...
            print(f"  - {log_filename}: Log embeddings {log_embeddings.shape} (300D Word2Vec vectors)")
...
def main():
    parser = argparse.ArgumentParser(description="Generate Word2Vec embeddings for log data using pre-trained vectors")
    ...
    if model is None:
        print("Failed to load pre-trained Word2Vec model. Exiting.")
        return
    ...
    for log_type in types_to_process:
        ...
        try:
            ...
            if vector_model is None:
                print(f"Skipping '{log_type}' due to missing Word2Vec vectors")
                continue
        ...
        try:
            ...
            if df_all.empty:
                ...
                if combined_vector_model is None:
                    print("Skipping combined processing due to missing Word2Vec vectors")
                else:
    ...
    spinner.start()
    spinner.succeed("Word2Vec embedding processing complete!")
